    ...
    responseCollector.on('collect', async response => {
        ...
        async function ticketmojibutton(userId, field, channel) {
            ...
            responseCollector.on('collect', async response => {
                ...
                // Utilisez une expression régulière pour extraire l'ID de l'emoji
...
const isNativeEmoji = /^<a?:.+:\d+>$/.test(content); // Vérifier si c'est une emoji native

if (!botEmoji && !isNativeEmoji) {
    // Le bot n'a pas accès à cet emoji et ce n'est pas une emoji native
    ...
    return;
        ...
        async function ticketbuttoncolor(userId, field, channel) {
            ...
            responseCollector.on('collect', async response => {
                const content = response.content.toLowerCase(); // Convertir en minuscules pour une correspondance insensible à la casse
